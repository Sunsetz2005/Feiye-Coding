...
- **其他目录项**（Notion、Slack、Outlook 等）在市场里显示「即将在 App 内连接」，不得要求用户安装 Open Connector。
- 目录字段 `connectKind`: GitHub、Gmail、Drive、Calendar 为 `in_app`，其余为 `coming`。探活失败或工具表为空时，不得标成已连接。
...
产品连接走 Host 适配器（GitHub PAT，以及 Gmail / Drive / Calendar 的 Google 登录）。loopback sidecar **不是** Cursor/Codex 式连接，市场文案不得引导用户下载或启动它。若仍设置环境变量，`SUNSETZ_OPEN_CONNECTOR_URL` 必须是 `http(s)://127.0.0.1`、`::1` 或 `localhost`。
...
连接器工具走现有权限条。`AcceptEdits` 不自动放行连接器写操作（含 `github_create_issue`、`gmail_create_draft`、`google-drive_create_file`、`google-calendar_create_event`）。`AlwaysApprove` 仍放行。只读 GitHub / Google 工具可走会话缓存。

## 测试入口

GitHub API 可用 `SUNSETZ_GITHUB_API_URL` 指向 mock。Gmail / Drive / Calendar 分别用 `SUNSETZ_GMAIL_API_URL`、`SUNSETZ_DRIVE_API_URL`、`SUNSETZ_CALENDAR_API_URL`；OAuth 用 `SUNSETZ_GOOGLE_OAUTH_*`。Open Connector 用 loopback HTTP mock。应用测试不得要求 grok 二进制。
