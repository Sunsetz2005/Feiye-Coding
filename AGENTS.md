...
- The setup wizard and home route must not require a Grok CLI binary. Host tools are `read_file`, `list_directory`, `grep`, `write_file`, `search_replace`, `run_command`, and parent-only `spawn_agent` / `agent_output` / `kill_agent` inside a trusted project root. Subagents are in-process, depth 1, and never create worktrees. Writes, replacements, and commands go through the existing permission dock; `AcceptEdits` auto-allows in-root writes and replacements only and never commands. Explicit Skills attach bounded `SKILL.md` text from Host-trusted skill directories; the product kernel remains `agent_loop`, not GROK_HOME or a grok binary. This slice does not add sandbox.
