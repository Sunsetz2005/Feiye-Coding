...
permission option mapping). Exercised by `src/acp_golden_test.rs` and the
session-manager timeline test via `cargo test --lib` — **no network, no real CLI**.
...
| `assistant_tool_timeline.json` | Assistant/tool phase journal ordering + in-place tool updates |
