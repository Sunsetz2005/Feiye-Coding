...
pub enum AcpEvent {
    ...
    /// pairs them with per-inference Runtime telemetry before presenting
...
impl AcpClient {
    pub fn use_mock() -> bool {
        crate::runtime_compat::use_mock_runtime()
    }
    ...
    match kind {
        ...
        "turn_completed" => {
            if let Some(usage) = update.get("usage") {
                    ...
                    .and_then(|models| {
                        if models.len() == 1 {
                            ...
                            models
                                .iter()
                                .max_by_key(|(_, body)| {
                                    body.get("inputTokens")
                                        .or_else(|| body.get("input_tokens"))
                                        .and_then(Value::as_u64)
                                        .unwrap_or(0)
                                })
                                .map(|(id, _)| id.clone())
                ...
                let input_tokens = read("inputTokens", "input_tokens");
                let output_tokens = read("outputTokens", "output_tokens");
                if input_tokens > 0 || output_tokens > 0 {
                    out.push(AcpEvent::Usage {
                        input_tokens,
                        output_tokens,
...
mod usage_update_tests {
    ...
    #[test]
    fn decodes_turn_completed_usage_and_model() {
        ...
        match &events[0] {
            AcpEvent::Usage {
                ...
                assert_eq!(*input_tokens, 76_334);
                assert_eq!(*output_tokens, 1_026);
                assert_eq!(*cached_read_tokens, 44_416);
                assert_eq!(*reasoning_tokens, 94);
...
pub struct AskUserOption {
    ...
    /// True only when the agent explicitly recommends this option.
    ///
    /// Some runtimes send a boolean (`recommended` / `isRecommended`), while
    /// older builds append the exact suffix ` (Recommended)` to the label.
...
fn parse_ask_user_options(v: &Value) -> Vec<AskUserOption> {
    ...
    for (i, item) in arr.iter().enumerate() {
        if let Some(s) = item.as_str() {
            let (label, recommended) = parse_recommended_label(s.trim());
            if label.is_empty() {
                continue;
            }
        ...
        let raw_label = obj
            ...
            .trim()
            .to_string();
        let (label, suffix_recommended) = parse_recommended_label(&raw_label);
        if label.is_empty() {
            continue;
        }
        let recommended = obj
            .get("recommended")
            .or_else(|| obj.get("isRecommended"))
...
/// Recognize only the canonical suffix emitted by older Runtime builds.
///
/// Deliberately do not treat case variants or arbitrary parenthesized copy as
/// protocol metadata: the visible label must otherwise be preserved exactly.
fn parse_recommended_label(label: &str) -> (String, bool) {
    const SUFFIX: &str = " (Recommended)";
    match label.strip_suffix(SUFFIX) {
        Some(base) if !base.trim().is_empty() => (base.trim_end().to_string(), true),
...
mod ask_user_question_tests {
    ...
    #[test]
    fn parses_explicit_recommended_flags_and_exact_legacy_suffix() {
        let params = json!({
            "question": "Which store?",
            "options": [
                { "label": "SQLite", "recommended": true },
                { "label": "Postgres", "isRecommended": true },
                "DuckDB (Recommended)",
                "Redis (recommended)",
        ...
        assert_eq!(options[0].label, "SQLite");
        assert!(options[0].recommended);
        assert_eq!(options[1].label, "Postgres");
        assert!(options[1].recommended);
        assert_eq!(options[2].label, "DuckDB");
        assert!(options[2].recommended);
        assert_eq!(options[3].label, "Redis (recommended)");
        assert!(!options[3].recommended);
        assert_eq!(options[4].label, "Recommended architecture");
        assert!(!options[4].recommended);
    }

    #[test]
    fn recommended_suffix_never_creates_an_empty_option() {
        let params = json!({
            "question": "Choose",
        ...
        assert_eq!(parsed.questions[0].options[0].label, "(Recommended)");
        assert!(!parsed.questions[0].options[0].recommended);
        assert_eq!(parsed.questions[0].options[1].label, "(Recommended)");
        assert!(!parsed.questions[0].options[1].recommended);
    }
}
