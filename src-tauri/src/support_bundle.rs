...
pub fn write_support_bundle(doctor_json: &str) -> Result<PathBuf, String> {
    ...
    let out = std::env::temp_dir().join(format!("sunsetz-support-{stamp}.zip"));
    ...
    let readme = "Sunsetz support bundle\n\
\n\
Contents:\n\
...
pub fn write_session_bundle(
    ...
    let out = std::env::temp_dir().join(format!("sunsetz-session-{short_id}-{stamp}.zip"));
    ...
    // ── agent/ — Sunsetz Runtime session trail ──────────────────────────────────
    ...
    let readme = format!(
        "Sunsetz session diagnostic bundle\n\
\n\
Session: {session_id}\n\
Title: {}\n\
Generated: {}\n\
...
- host/cli_probe.json — Sunsetz Runtime discovery\n\
- logs/ — recent App log files (if present)\n\
- agent/ — Sunsetz Runtime session trail (events, history, terminal logs)\n\
\n\
