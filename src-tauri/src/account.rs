...
fn display_from_subscription_code(code: &str) -> Option<String> {
    ...
    match lower.as_str() {
        "supergrokpro" | "supergrokheavy" | "heavy" => Some("Sunsetz Pro Heavy".into()),
        "supergrok" | "supergroklite" | "grokpro" => Some("Sunsetz Pro".into()),
...
fn display_from_jwt_tier(tier: i64) -> Option<String> {
    ...
    if tier >= 5 {
        Some("Sunsetz Pro Heavy".into())
    } else if tier >= 2 {
        Some("Sunsetz Pro".into())
    } else {
        None
    }
}
...
async fn fetch_subscription_meta(token: &str) -> SubscriptionMeta {
    let client = match reqwest::Client::builder()
        ...
        .user_agent("Sunsetz/0.1 (desktop; unofficial)")
        .build()
    {
        Ok(c) => c,
...
async fn fetch_billing_remote(token: &str) -> BillingSnapshot {
    ...
    BillingSnapshot {
        ...
        message: Some(
            "Could not fetch billing. Open Sunsetz usage on the web, or re-login via CLI.".into(),
        ),
        manage_url: USAGE_MANAGE_URL.into(),
        subscribe_url: SUBSCRIBE_URL.into(),
        ..Default::default()
    ...
    let billing = if refresh_billing {
        if let Some(token) = read_access_token() {
            ...
            BillingSnapshot {
                ...
                message: Some("Sign in with Sunsetz Runtime to load quota.".into()),
                manage_url: USAGE_MANAGE_URL.into(),
                subscribe_url: SUBSCRIBE_URL.into(),
                products: vec![],
                ..Default::default()
    ...
    let cli = match resolve_cli_path(manual_cli) {
        ...
        None => {
            return LoginResult {
                ok: false,
                method: method.into(),
                message: "Sunsetz Runtime not found. Install or set CLI path in Settings.".into(),
                device_url: None,
                device_code: None,
                profile: None,
            };
    ...
    fn maps_subscription_codes_to_display() {
        ...
        assert_eq!(
            resolve_subscription_display(&SubscriptionMeta {
                display: Some("Sunsetz Pro Heavy".into()),
