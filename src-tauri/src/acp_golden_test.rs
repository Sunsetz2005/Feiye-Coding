...
fn ask_user_question_parse_and_replies_match_fixture() {
    ...
    let recommended: Vec<bool> = parsed.questions[0]
        .options
        .iter()
        .map(|option| option.recommended)
        .collect();
    let expect_recommended: Vec<bool> = expect_q[0]["recommended"]
        .as_array()
        .unwrap()
        .iter()
        .map(|value| value.as_bool().unwrap())
        .collect();
    assert_eq!(recommended, expect_recommended);
