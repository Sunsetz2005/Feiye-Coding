//! Install / update Sunsetz Runtime CLI with multi-mirror download fallback.
//!
...
fn platform_triple() -> Result<(&'static str, &'static str), String> {
    let os = if cfg!(target_os = "macos") {
        ...
        return Err("Unsupported OS for Sunsetz Runtime auto-install".into());
    };
    let arch = if cfg!(target_arch = "aarch64") {
        ...
        return Err("Unsupported CPU architecture for Sunsetz Runtime auto-install".into());
    };
    Ok((os, arch))
}
...
async fn resolve_version(
    ...
    emit(
        ...
        progress(
            "resolving",
            "Resolving latest Sunsetz Runtime version…",
            Some(0.0),
            None,
            None,
        ),
    ...
    Err(format!(
        "Could not resolve Sunsetz Runtime version from any mirror. {}",
...
/// Download latest stable Sunsetz Runtime and install into `~/.grok`.
pub async fn install_cli_latest(app: AppHandle) -> Result<CliInstallResult, String> {
    ...
    emit(
        ...
        progress(
            "downloading",
            format!("Found Sunsetz Runtime v{version}"),
            Some(4.0),
    ...
    Ok(CliInstallResult {
        ...
        message: "Sunsetz Runtime installed".into(),
        sha256: Some(digest),
