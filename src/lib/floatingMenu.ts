...
import { useFloatingSurfacePresence } from "@/components/FloatingSurfaceProvider";
...
/**
 * Position a submenu beside the row that owns it.
 *
 * It prefers the reading direction (right), flips as one continuous surface
 * when that side cannot fit, then clamps both axes for small/zoomed windows.
 * Inputs and output are viewport coordinates because cascade panels are
 * portaled to `document.body`.
 */
export function computeCascadePos(
  anchor: DOMRect,
  ...
  const viewportWidth =
    typeof g.innerWidth === "number" ? g.innerWidth : 1024;
  const viewportHeight =
    typeof g.innerHeight === "number" ? g.innerHeight : 768;
  const availableWidth = Math.max(0, viewportWidth - margin * 2);
  const width = Math.min(Math.max(160, panel.width), availableWidth);
  const maxHeight = Math.max(0, viewportHeight - margin * 2);
  const renderedHeight = Math.min(Math.max(0, panel.height), maxHeight);
  const spaceRight = viewportWidth - margin - anchor.right - gap;
  const spaceLeft = anchor.left - margin - gap;
  const side: CascadeSide =
  ...
  const idealTop = anchor.top - 7;
  const top = Math.max(
    margin,
    Math.min(idealTop, viewportHeight - margin - renderedHeight),
  );
...
export interface UseFloatingMenuOptions {
  open: boolean;
  /** Optional stable debug identity. A generated per-instance id is the default. */
  surfaceId?: string;
...
export function useFloatingMenu({
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      onClose();
      // Escape reverses the opening interaction, so keyboard focus returns to
      // its spatial source. Pointer dismissal and ownership handoff keep the
      // focus selected by the user's new action.
      queueMicrotask(() => {
