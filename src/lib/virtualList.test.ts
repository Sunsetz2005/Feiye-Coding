...
describe("totalListHeight / itemOffset", () => {
  ...
  it("matches the sidebar row-height token used by CSS", () => {
    ...
    expect(H).toBe(34);
    expect(G).toBe(2);
    expect(css).toContain(`--sidebar-session-row-height: ${H}px`);
    expect(css).toContain("height: var(--sidebar-session-row-height)");
    expect(css).toContain("min-height: var(--sidebar-session-row-height)");
    expect(css).toContain("max-height: var(--sidebar-session-row-height)");
  });

  it.each([
    { count: 31, virtualized: false },
    { count: 32, virtualized: true },
    { count: 33, virtualized: true },
    { count: 100, virtualized: true },
  ])(
    "keeps $count rows geometrically contiguous at the virtualization boundary",
    ({ count, virtualized }) => {
      expect(count >= SIDEBAR_VIRTUALIZE_THRESHOLD).toBe(virtualized);
      expect(totalListHeight(count, H, G)).toBe(
        count * H + (count - 1) * G,
      );
      expect(itemOffset(count - 1, H, G) + H).toBe(
        totalListHeight(count, H, G),
      );
    },
  );
});
...
describe("scrollTopForIndex", () => {
  ...
  it("accounts for listOffsetTop within a nested group", () => {
    ...
    // Item absolute top = 200 + 5*(34+2) = 380; viewport 300 → need scroll
