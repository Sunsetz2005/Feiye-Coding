...
/**
 * Runtime compatibility mark paths (currentColor) — follows theme via CSS color.
 * Dark: light glyph; light: dark glyph (no invert filter needed).
 */
export function IconRuntimeMark({
  size = 22,
  title = "Sunsetz Runtime",
  className = "",
}: IconProps) {
  const classes = ["g-icon", "g-icon--runtime-mark", className]
    .filter(Boolean)
    .join(" ");
  return (
    <span
...
export const IconSend = wrap(TbArrowUp);
