...
# Tauri and Cargo both use the Sunsetz product name.
find_release_exe() {
  local name
  for name in Sunsetz.exe sunsetz.exe; do
...
STAGE="dist-portable/Sunsetz_${VER}_x64-portable"
rm -rf dist-portable
mkdir -p "$STAGE"
# Always ship as Sunsetz.exe for end users.
cp "$EXE" "$STAGE/Sunsetz.exe"
...
(stage / "README-portable.txt").write_text(
    f"""Sunsetz portable (绿色版) v{ver}
================================
1. 解压本目录到任意位置（无需安装）。
2. 双击 Sunsetz.exe 运行。
3. 需要系统已安装 Microsoft Edge WebView2 Runtime（Win10/11 通常已自带）。
4. 真 Agent 能力仍需 Sunsetz Runtime 并完成登录。
5. SmartScreen 可能提示未知发布者 → 更多信息 → 仍要运行。

Extract anywhere and run Sunsetz.exe. WebView2 and Sunsetz Runtime are required for agent sessions.
""",
    encoding="utf-8",
)
...
OUT="Sunsetz_${VER}_x64-portable.zip"
# Prefer zip; on Windows Git Bash it is usually present. Fallback to PowerShell Compress-Archive.
if command -v zip >/dev/null 2>&1; then
  (cd dist-portable && zip -r "../${OUT}" "Sunsetz_${VER}_x64-portable")
else
  powershell.exe -NoProfile -Command \
    "Compress-Archive -Path 'dist-portable/Sunsetz_${VER}_x64-portable' -DestinationPath '${OUT}' -Force"
fi
ls -lah "$OUT"
